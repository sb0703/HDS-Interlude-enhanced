...
function mergeWorkingDetails(existing, drafts, entries, now) {
  ...
  for (const draft of drafts) {
    ...
    const sourceEntryIds = Array.isArray(draft.sourceEntryIds) ? [...new Set(draft.sourceEntryIds.filter((id) => Number.isSafeInteger(id) && evidence.has(id)))].slice(0, 8) : [];
    ...
    if (draft.status !== void 0 && draft.status !== "active") continue;
    const value = typeof draft.value === "string" ? draft.value.slice(0, 300).trim() : "";
    if (!value) continue;
    const expiresAt = typeof draft.expiresAt === "string" && Number.isFinite(Date.parse(draft.expiresAt)) ? draft.expiresAt : current?.expiresAt;
...
function validateNarrativeReview(value, request) {
  ...
  if (request.requireSemanticChecks) {
    ...
    const checks = [];
    for (const kind of ["time", "progression"]) {
...
// src/meta.ts
var HDS_INTERLUDE_VERSION = "0.1.5-beta8-m6.custom.4";
...
${message.content}`).join("\n\n");
  ...
  async persistTimelineDirectorRetry(story, from, now, failures) {
  ...
  async reviewCandidate(context, raw, retrievalHints) {
    ...
    if (phase === "advance" || phase === "user-message") {
      allowedDeliveries.push(...decision.crossConversationActions.filter((action) => action.mode === "immediate").map((action) => ({ target: action.participantId, content: action.content })));
    }
  ...
  async tryDecide(story, participant, phase, from, now, userMessage, dueIntents, supersededIntents = [], groupContext, images = [], chatCapabilities, quotedMessages = [], stickerCatalog = [], turnQueryEmbedding, visualObservations, onEarlyReply, options = {}) {
    ...
    try {
      ...
      return {
        ...
        immediateObservations,
        timelinePlan,
        failureReason: error instanceof Error && error.message.startsWith("Narrative repetition guard") ? "repetition" : "provider"
      };
    }
  }
   ...
   * automatic window. */
  async persistTimelineSceneAnchor(storyId, prose, now) {
...
${prose.slice(-Math.max(1, this.memoryConfig.sceneSummaryCharacters - 120))}`;
  ...
  async analyzeAlterSystem(storyId, phase, participantId = "") {
    ...
    try {
      const scripts = (await this.recentTimelineEntries(story.id, 50)).filter((entry) => entry.kind === "script" && entry.content.trim() && (!entry.participantId || entry.participantId === participantId)).slice(-10).map((entry) => ({ content: entry.content.slice(0, 4e3), occurredAt: entry.occurredAt.toISOString() }));
      this.reportOperation(
        "standard",
        "info",
        story,
  ...
  async ensureConfiguredSchedulePreplan(story, now) {
    ...
    const fingerprint = (0, import_node_crypto.createHash)("sha256").update(JSON.stringify({
      profile,
      timezone
    })).digest("hex");
  ...
  async rebuildLiveOverlayState(story, now) {
    const [applied, snapshots] = await Promise.all([
      this.dbGetTimeline("interlude_state_patch", { storyId: story.id, status: "applied" }),
...
function normalizeVisibleMessageContent(value, maxCharacters, separator = "<sep/>") {
  return normalizeMessageSeparators(value, separator).replace(/[\[【](?:表情包?|图片|动图|GIF)[\]】]/gi, "").replace(/[\[【](?:流汗|微笑|笑哭|尴尬|爱心|惊讶|流泪|委屈)[\]】]/gi, "").trim().slice(0, Math.max(1, maxCharacters));
}
...
function automaticDeliveryFromPayload(value) {
  const record3 = isRecord4(value) && isRecord4(value.automaticDelivery) ? value.automaticDelivery : void 0;
...
function normalizeConversationAction(value, runtime, permittedParticipantIds, currentParticipantId, now = /* @__PURE__ */ new Date(), proactive = false) {
  ...
  const content = typeof value.content === "string" ? normalizeMessageSeparators(value.content, runtime.messageSeparator).trim().slice(0, runtime.maxMessageCharacters) : "";
  if (!content) return void 0;
...
function normalizeStoryState(value) {
  ...
  const continuity = isRecord4(record3.continuitySnapshot) ? normalizeContinuitySnapshot(record3.continuitySnapshot) : void 0;
  return {
    timelineBoundary: normalizeTimelineBoundary(record3.timelineBoundary),
    settingOverlay: {
    ...
    activeSceneId: typeof record3.activeSceneId === "number" ? record3.activeSceneId : void 0,
    activeArcId: typeof record3.activeArcId === "number" ? record3.activeArcId : void 0,
    continuitySnapshot: continuity,
    narrativeUpdateCount: Math.max(0, Math.floor(typeof record3.narrativeUpdateCount === "number" ? record3.narrativeUpdateCount : 0)),
    lastContinuityUpdateAt: typeof record3.lastContinuityUpdateAt === "string" ? record3.lastContinuityUpdateAt : void 0,
    continuityDirty: record3.continuityDirty === true,
    scheduleProfileFingerprint: typeof record3.scheduleProfileFingerprint === "string" ? record3.scheduleProfileFingerprint : void 0,
    chatRhythm: normalizeChatRhythmState(record3.chatRhythm),
    ...
    automation: {
      ...
      timelineRetryAt: typeof automation.timelineRetryAt === "string" ? automation.timelineRetryAt : void 0,
      timelineRetryFrom: typeof automation.timelineRetryFrom === "string" ? automation.timelineRetryFrom : void 0,
      timelineDirectorFailures: typeof automation.timelineDirectorFailures === "number" ? Math.max(0, Math.floor(automation.timelineDirectorFailures)) : void 0
    }
  };
}
...
var TimelineDirector = import_koishi2.Schema.object({
  enabled: import_koishi2.Schema.boolean().default(true).description("\u542F\u7528\u65F6\u95F4\u5BFC\u6F14\u3002\u8FDE\u7EED\u5931\u8D25\u4F1A\u6307\u6570\u9000\u907F\uFF0C\u516D\u6B21\u540E\u4E34\u65F6\u964D\u7EA7\u4E3A\u7ECF\u8FC7\u7EDF\u4E00\u5BA1\u6838\u7684\u5B88\u6052\u63A8\u8FDB\u3002")
});
