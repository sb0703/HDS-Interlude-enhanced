...
var OpenAICompatibleNarrator = class {
  ...
  async reviewNarrative(request) {
    ...
    try {
      const requestOnce = async (requestBody) => provider.zhipuOfficial ? requestZhipuStreaming(provider.endpoint, { ...requestBody, stream: true, thinking: { type: "enabled" }, reasoning_effort: provider.reasoningEffort ?? "high" }, headers, void 0, collect) : (async () => {
        const response = await this.ctx.http.post(provider.endpoint, withDeepSeekThinking(provider, requestBody), { headers, timeout: compactConfig?.timeout || route.timeout || provider.timeout });
        collect(response?.usage);
        return extractChatText(response);
      })();
      const confirmReportedTimes = async (review2) => {
        ...
        return reportedTimes ? { ...review2, reportedTimes } : void 0;
      };
      const text3 = await requestOnce(body);
      if (!text3) return void 0;
      let parsed;
      let failure = "invalid-json";
      try {
        parsed = parseJsonResponse(text3, "Narrative consistency review");
        const review2 = normalizeNarrativeReview(parsed, request);
        if (review2) return await confirmReportedTimes(review2).catch(() => void 0);
      ...
      if (!repairedText) return void 0;
      const repaired = parseJsonResponse(repairedText, "Narrative consistency review repair");
      ...
      return review ? await confirmReportedTimes(review).catch(() => void 0) : void 0;
    } catch (error) {
...
${message.content}`).join("\n\n");
  ...
  async reviewCandidate(context, raw, retrievalHints) {
    ...
    const request = { context, candidate: raw, allowedDeliveries, alreadyDelivered, retrievalHints, requireSemanticChecks: true, evidenceCharacterBudget: this.config.model.consistencyReviewHistoryCharacters };
