...
// .local-hdsi/koishi-app/node_modules/qface/lib/data.json
var require_data = __commonJS({
  ".local-hdsi/koishi-app/node_modules/qface/lib/data.json"(exports2, module2) {
    module2.exports = [
      {
        QSid: "0",
        QDes: "/\u60CA\u8BB6",
...
// .local-hdsi/koishi-app/node_modules/qface/lib/index.js
var require_lib = __commonJS({
  ".local-hdsi/koishi-app/node_modules/qface/lib/index.js"(exports2, module2) {
...
__export(src_exports, {
  ...
  name: () => name,
  narrativeAutomaticSceneLoop: () => narrativeAutomaticSceneLoop,
  narrativeClockConflict: () => narrativeClockConflict,
  narrativeHasProgression: () => narrativeHasProgression,
  narrativeImageAttachable: () => narrativeImageAttachable,
  narrativeLocationSequence: () => narrativeLocationSequence,
  narrativeRecoveryStillSimilar: () => narrativeRecoveryStillSimilar,
  narrativeScheduleWindowConflict: () => narrativeScheduleWindowConflict,
  narrativeSegmentOverlap: () => narrativeSegmentOverlap,
  narrativeTextSimilarity: () => narrativeTextSimilarity,
  narrativeTimelinePlanConflict: () => narrativeTimelinePlanConflict,
...
function recentContinuityContext(entries, now) {
  ...
  const narratedBeats = windows.flatMap((entry) => {
    const plan = entry.metadata.timelinePlan;
    if (!Array.isArray(plan.beats)) return [];
    return plan.beats.filter((beat) => record(beat) && ["state", "activity", "thought"].includes(String(beat.kind)) && typeof beat.at === "number" && Number.isFinite(beat.at) && beat.at >= 0 && beat.at <= 1 && typeof beat.summary === "string" && !!beat.summary.trim()).sort((left, right) => left.at - right.at).slice(0, 4).map((beat) => ({ entryId: entry.id, participantId: entry.participantId, windowEndedAt: entry.occurredAt.toISOString(), kind: beat.kind, summary: beat.summary.trim().slice(0, 180) }));
  });
...
function narrativeReviewPrompt() {
  return [
...
function toNarrativeReviewPayload(request) {
  ...
  const evidence = [
    ...
    { ref: "current-event", value: {
      ...
      userReportedTimes: context.userReportedTimes
    } },
...
function narrativeReviewRepairPrompt(reason) {
  return [
...
function extractUserReportedTimes(content, now, timezone) {
  ...
  for (const match of content.matchAll(pattern)) {
    ...
    if (match[2] === "." && !/(?:今天|昨天|明天|今晚|早上|上午|下午|晚上|凌晨|中午|时间|几点|从|到|约|在|我)\s*$/u.test(before) && !/^\s*(?:开始|结束|出发|到达|见|开会|吃|睡|起床)/u.test(after)) continue;
    ...
    if (dayMatch?.[1]) {
      ...
      const offset = { \u524D\u5929: -2, \u6628\u5929: -1, \u6628\u665A: -1, \u4ECA\u5929: 0, \u4ECA\u665A: 0, \u660E\u5929: 1, \u660E\u665A: 1, \u540E\u5929: 2 };
      const dateValue2 = /* @__PURE__ */ new Date(`${local.date}T00:00:00Z`);
      dateValue2.setUTCDate(dateValue2.getUTCDate() + offset[dayMatch[0]]);
      date = dateValue2.toISOString().slice(0, 10);
    }
    const statement = content.slice(Math.max(0, index - 64), Math.min(content.length, index + match[0].length + 96)).trim();
    const unsupportedDate = /上周|下周|上个月|下个月|去年|明年|周[一二三四五六日天]|星期[一二三四五六日天]/u.test(before) || !dayMatch && /\d{1,2}月\d{1,2}日/u.test(before);
    if (unsupportedDate) {
      results.push({ relation: "ambiguous", statement });
      continue;
    }
...
function narrativeClockConflict(script, from, now, timezone) {
  const text3 = (script ?? "").replace(/“[^”]*”|「[^」]*」|"[^"]*"/gu, "");
  ...
  for (const sentence of text3.split(/[。！？!?;；\n]/u)) {
    ...
    for (const match of sentence.matchAll(/(?<!\d)(?:[01]?\d|2[0-3])[:：][0-5]\d(?!\d)/gu)) {
      ...
      const inInterval = now.getTime() - from.getTime() >= 864e5 || (start.date === endpoint.date ? observed >= start.time.slice(0, 5) && observed <= expected : observed >= start.time.slice(0, 5) || observed <= expected);
      if (explicitNow ? observed === expected : inInterval) continue;
      const minutes = (clock2) => Number(clock2.slice(0, 2)) * 60 + Number(clock2.slice(3, 5));
...
function schedulePreplanWindow(record3, now, timezone, hours = 12, config = DEFAULT_SCHEDULE_PREPLAN_CONFIG) {
  ...
  const blocks = [];
  for (const day of record3.materializedDays) {
...
${message.content}`).join("\n\n");
  ...
  async reviewCandidate(context, raw, repetitionSignal, clockSignal) {
    ...
    const request = { context, candidate: raw, allowedDeliveries, alreadyDelivered, repetitionSignal, clockSignal, evidenceCharacterBudget: this.config.model.consistencyReviewHistoryCharacters };
  ...
  async tryDecide(story, participant, phase, from, now, userMessage, dueIntents, supersededIntents = [], groupContext, images = [], chatCapabilities, quotedMessages = [], stickerCatalog = [], turnQueryEmbedding, visualObservations, onEarlyReply, options = {}) {
    ...
    try {
      ...
      for (let attempt = 0; ; attempt++) {
        ...
        const review = reviewEnabled ? await this.reviewCandidate(reviewContext, decision, repetition, clockConflict) : void 0;
        if (reviewEnabled ? review?.verdict === "pass" : !clockConflict?.explicitNow) {
          this.reportOperation("diagnostic", "debug", story, phase, reviewEnabled ? "\u7EDF\u4E00\u903B\u8F91\u5BA1\u6838\u901A\u8FC7\uFF0C\u672C\u8F6E\u5141\u8BB8\u8FDB\u5165\u6301\u4E45\u5316\u4E0E\u6295\u9012\u68C0\u67E5" : "\u7EDF\u4E00\u903B\u8F91\u5BA1\u6838\u5DF2\u5173\u95ED\uFF0C\u4EC5\u68C0\u67E5\u901A\u7528\u65F6\u95F4\u53CA\u7ED3\u6784\u8FB9\u754C");
          break;
        }
        const recovery = [
  ...
  async narrativeRepetition(storyId, participantId, script, now) {
    const candidate = script?.trim() ?? "";
    if (!candidate) return {
      repeated: false,
      partialRepeated: false,
      stagnantTurns: 0,
      similarity: 0,
      coreSimilarity: 0,
    ...
    const visible = rows.filter((entry) => entry.kind === "script" && entry.content.trim() && (!participantId || !entry.participantId || entry.participantId === participantId));
    const comparisons = visible.map((entry) => {
      const ageSeconds2 = Math.max(0, now.getTime() - entry.occurredAt.getTime()) / import_koishi.Time.second;
    ...
    }).filter((item) => item.ageSeconds <= 90 * 60 && (item.score >= 0.58 || item.coreScore >= 0.72 || item.segmentOverlap.ratio >= 0.56 && item.segmentOverlap.matchedSegments >= 2)).sort((left, right) => right.score - left.score || left.ageSeconds - right.ageSeconds);
    const stagnant = comparisons.filter((item) => !item.progressed && (item.coreScore >= 0.68 || item.segmentOverlap.ratio >= 0.62 && item.segmentOverlap.matchedSegments >= 2));
    const match = comparisons.find((item) => !item.progressed && item.ageSeconds <= 45 * 60 && (item.coreScore >= 0.78 || item.score >= 0.68 || item.segmentOverlap.ratio >= 0.62 && item.segmentOverlap.matchedSegments >= 2));
    const partialMatch = comparisons.find((item) => item.ageSeconds <= 45 * 60 && item.coreScore >= 0.6 && item.segmentOverlap.ratio >= 0.7 && item.segmentOverlap.matchedSegments >= 3 && item.segmentOverlap.novelRatio <= 0.38);
  ...
  async persistSchedulePreplanReview(story, review, proposal, now) {
    ...
    if (live?.revision !== review.current?.revision || liveStory.setting.character.profile !== story.setting.character.profile || liveStory.setting.timezone !== story.setting.timezone) {
...
var NARRATIVE_PROGRESSION_MARKERS = [
  "\u8F6E\u5230",
  "\u4E0B\u4E00\u4F4D",
  "\u5F80\u524D\u632A",
  "\u6392\u5230",
...
function narrativeCoreSimilarity(left, right) {
...
function narrativeTimelinePlanConflict(script, plan, interaction) {
  const planned = plan?.beats.map((beat) => beat.summary).join(" ") ?? "";
  if (!planned.trim()) return void 0;
...
function narrativeScheduleWindowConflict(script, schedule, now, timezone, interaction) {
  if (!schedule || schedule.timezone !== timezone) return void 0;
...
${interaction?.reply?.mode === "immediate" ? interaction.reply.content ?? "" : ""}`.trim();
  ...
  if (justifiedDelay.test(content)) return void 0;
...
function narrativeAutomaticSceneLoop(script, from, now) {
  const content = script?.trim() ?? "";
  if (!content) return void 0;
  const locations = narrativeLocationSequence(content);
  const elapsedMinutes = Math.max(0, Math.round((now.getTime() - from.getTime()) / import_koishi.Time.minute));
  ...
  if (!returnedToStart && transitions <= transitionLimit) return void 0;
  return { locations, transitions, elapsedMinutes, returnedToStart };
}
function narrativeLocationSequence(value) {
  const patterns = [
...
function narrativeRecoveryStillSimilar(repetition) {
  return repetition.coreSimilarity >= 0.72 && repetition.segmentOverlapCount >= 2 && repetition.segmentOverlapRatio >= 0.62 || repetition.coreSimilarity >= 0.6 && repetition.segmentOverlapRatio >= 0.7 && repetition.segmentOverlapCount >= 3;
}
...
function narrativeHasProgression(current, previous) {
  ...
  const novelLength = currentSegments.filter((segment) => !previousSegments.some((previousSegment) => narrativeTextSimilarity(segment, previousSegment) >= 0.68)).reduce((total, segment) => total + segment.length, 0);
  const totalLength = currentSegments.reduce((total, segment) => total + segment.length, 0);
  ...
  return newMarkers > 0 && (novelLength >= 18 || totalLength > 0 && novelLength / totalLength >= 0.18) || newMarkers >= 2 && endingChanged;
}
...
0 && (module.exports = {
  ...
  name,
  narrativeAutomaticSceneLoop,
  narrativeClockConflict,
  narrativeHasProgression,
  narrativeImageAttachable,
  narrativeLocationSequence,
  narrativeRecoveryStillSimilar,
  narrativeScheduleWindowConflict,
  narrativeSegmentOverlap,
  narrativeTextSimilarity,
  narrativeTimelinePlanConflict,
