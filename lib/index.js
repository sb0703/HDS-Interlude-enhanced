...
function timelineDirectorPrompt() {
  return [
    ...
    "Historical script prose can itself contain a mistaken future clock. Its occurredAt is the host-owned event time; a clock, daypart, weekday or later completed activity inside that prose never moves the current interval. Before returning each beat, compute its host-local time from interval.fromLocal + at \xD7 elapsed duration. If the beat summary names a later clock or daypart, remove that event or keep only the in-window state. A ten-minute noon interval cannot contain a completed evening departure even if recentEntries says it happened.",
...
function coerceTimelinePosition(value) {
  ...
  const parsed = Number(percent ? text3.slice(0, -1).trim() : text3);
