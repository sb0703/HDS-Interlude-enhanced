...
var OpenAICompatibleImageGenerator = class {
  ...
  async generate(prompt, options = {}) {
    ...
    const text2 = prompt.trim().slice(0, Math.max(1, this.config.maxPromptCharacters));
    if (!text2) throw new Error("\u8BF7\u63D0\u4F9B\u56FE\u7247\u63CF\u8FF0\u3002");
    ...
    if (this.config.mode === "dashscope-qwen-image") {
      ...
      return this.generateDashscopeQwenImage(endpoint, apiKey, model, text2, size, referenceImages, options.characterAppearance);
    }
    const body = {
    ...
    const response = await this.ctx.http.post(endpoint, body, {
      headers: {
  ...
  async generateDashscopeQwenImage(endpoint, apiKey, model, prompt, size, referenceImages = [], appearance = "") {
      ...
      input: {
        messages: [{
          ...
          content: [
            ...
            {
              text: referenceImages.length ? `\u56FE1\u662F\u4E3B\u89D2\u552F\u4E00\u7684\u4EBA\u7269\u8EAB\u4EFD\u57FA\u51C6\uFF0C\u5FC5\u987B\u4FDD\u6301\u56FE1\u7684\u8138\u578B\u3001\u4E94\u5B98\u6BD4\u4F8B\u3001\u5E74\u9F84\u611F\u3001\u53D1\u9645\u7EBF\u548C\u53EF\u89C1\u8FA8\u8BC6\u7279\u5F81\uFF0C\u4E0D\u5F97\u91CD\u65B0\u8BBE\u8BA1\u6210\u76F8\u4F3C\u4F46\u4E0D\u540C\u7684\u4EBA\u3002${referenceImages.length > 1 ? `\u56FE2${referenceImages.length > 2 ? "\u3001\u56FE3" : ""}\u4EC5\u8865\u5145\u540C\u4E00\u4E3B\u89D2\u7684\u4F53\u578B\u3001\u53D1\u578B\u6216\u670D\u88C5\u7EC6\u8282\uFF1B\u53D1\u751F\u51B2\u7A81\u65F6\u4EE5\u56FE1\u7684\u4EBA\u8138\u8EAB\u4EFD\u4E3A\u51C6\u3002` : ""}\u8FD9\u4E9B\u53C2\u8003\u56FE\u53EA\u7EA6\u675F\u4E3B\u89D2\u672C\u4EBA\uFF1B\u82E5\u753B\u9762\u8FD8\u6709\u5176\u4ED6\u4EBA\u7269\uFF0C\u4E0D\u5F97\u628A\u4E3B\u89D2\u5916\u8C8C\u590D\u5236\u7ED9\u4ED6\u4EEC\u3002${appearance.trim() ? `\u4E3B\u89D2\u7684\u56FA\u5B9A\u4F53\u8C8C\u7279\u5F81\uFF1A${appearance.trim().slice(0, 1200)}\u3002` : ""}\u5728\u4FDD\u6301\u4E3B\u89D2\u8EAB\u4EFD\u4E0D\u53D8\u7684\u524D\u63D0\u4E0B\u5B8C\u6210\u4EE5\u4E0B\u753B\u9762\uFF1A${prompt}` : prompt
            }
          ]
        }]
      },
...
var OpenAICompatibleNarrator = class {
  ...
  async requestCanonReview(provider, request, decision, overrides) {
    ...
    if (!text2) throw new Error("Canon guard returned an empty response.");
    return normalizeCanonReview(parseJsonResponse(text2, "Canon guard"));
  }
};
...
function systemPrompt(phase, mainPrompt, formatPrompt, fixedPrompt, baseStylePrompt, storyStylePrompt, refreshContinuity = false, alterEnabled = false, agencyEnabled = false, perspectiveEnabled = false, outputRecovery = false, chatCapabilities, hasQuotedMessage = false, stickerCatalog, canonRecovery = []) {
  return [
...
function runtimeLogProfile(config) {
    ...
    const accepted = await this.serial(story.id, async () => {
      ...
      if (userInput.quote?.senderId === String(session.selfId ?? "")) {
        await this.appendEntry(current.id, {
          kind: "quoted-character-message",
          actor: "character",
          content: userInput.quote.content,
          ...
          occurredAt: userInput.quote.occurredAt ?? now.toISOString(),
          metadata: {
            visible: true,
            quoted: true,
            sourceMessageId: userInput.quote.messageId,
...
${message.content}`).join("\n\n");
  ...
  async sendSticker(story, session, channelId, asset) {
    ...
    if (!relativePath || relativePath === ".." || relativePath.startsWith(`..${import_node_path2.sep}`) || relativePath.includes(":")) return;
    try {
  ...
  async generateImage(prompt, options) {
    ...
    this.reportStandaloneOperation(
      "standard",
      "info",
      "\u56FE\u7247\u751F\u6210\u5B8C\u6210 \u4E3B\u4F53=%s \u8DEF\u7531=%s \u6A21\u578B=%s",
      options?.subject || "non-person",
      characterReference ? "character-reference" : "standard",
      model || "unknown"
    );
    return result;
  }
...
function narrativeImageAttachable(imageGenerationEnabled, interaction, crossActions) {
  return imageGenerationEnabled && (interaction?.reply.mode === "immediate" && !!interaction.reply.content || crossActions.some((action) => action.mode === "immediate"));
}
function normalizeNarrativeImageGeneration(value) {
  if (!isRecord3(value) || typeof value.prompt !== "string") return void 0;
  const prompt = clip(value.prompt, 2e3).trim();
  const subject = normalizeImageSubject(value.subject);
  return prompt && subject ? { prompt, subject } : void 0;
}
function normalizeImageSubject(value) {
  return value === "protagonist" || value === "other-person" || value === "non-person" ? value : void 0;
}
function characterAppearanceFromProfile(profile) {
  ...
  return clip(section, 1200).trim();
}
function normalizeContinuitySnapshot(value) {
  if (!isRecord3(value)) return void 0;
  const text2 = (item, limit) => typeof item === "string" ? clip(item, limit).trim() : "";
  const list = (item, limit) => Array.isArray(item) ? item.map((value2) => text2(value2, limit)).filter(Boolean).slice(0, 5) : [];
...
function registerCommands(ctx, service) {
  ...
  ctx.command("interlude.image <prompt:text>", "\u7BA1\u7406\u5458\uFF1A\u4F7F\u7528\u72EC\u7ACB\u914D\u7F6E\u7684\u6587\u751F\u56FE\u6A21\u578B\u751F\u6210\u5E76\u53D1\u9001\u4E00\u5F20\u56FE\u7247").action(async ({ session }, prompt) => {
    ...
    if (!text2) return "\u8BF7\u63D0\u4F9B\u56FE\u7247\u63CF\u8FF0\uFF0C\u4F8B\u5982\uFF1Ainterlude.image \u96E8\u591C\u4FBF\u5229\u5E97\u95E8\u53E3\uFF0C\u4E00\u53EA\u6A58\u732B\u8DB4\u5728\u6696\u5149\u4E0B\u3002";
    try {
      const image = await target.generateImage(text2);
