...
test('compiled writing context retains local rhythm guidance and execution evidence', () => {
  const rhythm = { state: '近期回复结构接近', drift: '按当前信息自然组织文字，不强行拆段' }
  const result = compileNarrativeContext({ chatRhythm: rhythm, deliveryReality: [{ sourceEntryId: 1 }] }, undefined, undefined)
  assert.deepEqual(result.ongoingThreads.chatRhythm, rhythm)
