...
test('host clock rejects a later daypart inside a ten-minute noon window', () => {
  const from = new Date('2026-09-15T04:32:46Z')
  const now = new Date('2026-09-15T04:42:46Z')
  const invalid = normalizeTimelinePlan({ beats: [{ at: 0.5, kind: 'activity', summary: '周走出行政楼，成都傍晚的空气扑面而来，走向停车场。' }] })!
  assert.match(timelinePlanWindowIssues(invalid, from, now, 'Asia/Shanghai')[0], /evening/)
  const valid = normalizeTimelinePlan({ beats: [{ at: 0.5, kind: 'activity', summary: '继续在办公室处理材料。' }] })!
  assert.deepEqual(timelinePlanWindowIssues(valid, from, now, 'Asia/Shanghai'), [])
})

test('host clock uses local timezone and relative beat position across midnight', () => {
  const from = new Date('2026-09-15T13:55:00Z')
  const now = new Date('2026-09-15T14:05:00Z')
  const plan = normalizeTimelinePlan({ beats: [{ at: 1, kind: 'state', summary: '夜间22:05仍在整理行李。' }] })!
  assert.deepEqual(timelinePlanWindowIssues(plan, from, now, 'Asia/Shanghai'), [])
  assert.ok(timelinePlanWindowIssues(plan, from, now, 'UTC').length > 0)
})
