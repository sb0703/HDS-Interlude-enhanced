...
test('ledger read and write failures do not interrupt confirmation or remaining bubble scheduling', async () => {
  for (const failure of ['read', 'write']) {
    ...
    const warnings: any[] = []
    const service: any = {
    dbGetTimeline(...args: any[]) { return this.dbGet(...args) },
...
test('confirmed delivery persists through the real timeline scope guard', async () => {
  const value = commit()
  const event = findOutgoingScriptEvent(value, 'alice')!
  const entry: any = { id: 42, storyId: 'story:1', metadata: { commitId: value.commitId, deliveryActions: createScriptDeliveryActions(value) } }
  const host: any = {
    dbGetTimeline: (InterludeService.prototype as any).dbGetTimeline,
    dbGet: async (table: string, query: any) => {
      if (table === 'interlude_story') return [{ state: {} }]
      assert.equal(query.storyId, 'story:1')
      assert.equal(query.id, 42)
      return [entry]
    },
    dbSet: async (_table: string, _query: any, patch: any) => Object.assign(entry, patch),
    reportStandalone: () => assert.fail('confirmed delivery must update its ledger'),
  }
  for (const segmentIndex of [0, 1]) await (InterludeService.prototype as any).updateScriptDeliveryOutcome.call(
    host, 'story:1', { commitId: value.commitId, eventId: event.eventId, scriptEntryId: 42, segmentIndex }, 'delivered', new Date())
  const action = entry.metadata.deliveryActions.find((action: any) => action.eventId === event.eventId)
  assert.equal(action.status, 'delivered')
  assert.ok(action.segments.every((segment: any) => segment.status === 'delivered'))
})
