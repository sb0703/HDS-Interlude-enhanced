...
import { authoredActionRecoveryDraft, hasUnresolvedAuthoredActions, recoverExplicitAuthoredMirror, resolveAuthoredActions } from '../src/script/authored-actions'
...
test('bad explicit ids and duplicate authored ids never create guessed outgoing text', () => {
  for (const script of ['她想问问。', '<say id="r">甲</say><say id="r">乙</say>']) {
    ...
    assert.equal(hasUnresolvedAuthoredActions(result), true)
    assert.equal(result.interaction?.reply.content, undefined)
  }
  ...
  assert.equal(hasUnresolvedAuthoredActions(forged), true)
  assert.equal(forged.interaction?.reply.content, undefined)
})

test('recovery retains the original tagged provider object through repeated normalization', () => {
  const raw: NarrativeDecision = { script: '她写下<say id="r">原始回复</say>。', interaction: { seen: true, reply: { mode: 'immediate', actionId: 'wrong', content: '原始镜像' } } }
  const original = JSON.stringify(raw)
  let resolved = raw
  for (let i = 0; i < 4; i++) resolved = resolveAuthoredActions(resolved)
  assert.equal(authoredActionRecoveryDraft(resolved), raw)
  assert.equal(JSON.stringify(authoredActionRecoveryDraft(resolved)), original)
  assert.match(authoredActionRecoveryDraft(resolved).script!, /<say id="r">/)
})

test('only an exact explicitly authored multi-bubble mirror repairs a mismatched id locally', () => {
  const raw: NarrativeDecision = { script: '她写下<say id="first">你好。</say>，又写下<say id="second">请稍等。</say>。',
    interaction: { seen: true, reply: { mode: 'immediate', actionId: 'missing', content: '你好。<sep/>请稍等。' } } }
  const resolved = resolveAuthoredActions(raw)
  const recovered = recoverExplicitAuthoredMirror(resolved)!
  assert.equal(recovered.interaction?.reply.content, raw.interaction?.reply.content)
  assert.equal(recovered.interaction?.reply.actionId, undefined)
  assert.equal(hasUnresolvedAuthoredActions(recovered), false)
  assert.equal(recovered.script, '她写下你好。，又写下请稍等。。')
  for (const changed of [
    { ...raw, interaction: { seen: true, reply: { mode: 'immediate' as const, actionId: 'missing', content: '你好。<sep/>另一个答案。' } } },
    { ...raw, groupReply: { mode: 'immediate' as const, content: '群里另一个人' } },
    { ...raw, script: '她写下<say id="first">你好。</say>，又写下<say id="first">请稍等。</say>。' },
  ]) assert.equal(recoverExplicitAuthoredMirror(resolveAuthoredActions(changed)), undefined)
})

test('authored markup accepts equivalent quotes and whitespace without inferring prose', () => {
  for (const tag of ["<say id='r'>原话</say>", '<say  id = "r" >原话</say >', '<SAY\nid="r">原话</SAY>']) {
    const resolved = resolveAuthoredActions({ script: `她发来${tag}。`, interaction: { seen: true, reply: { mode: 'immediate', actionId: 'r' } } })
    assert.equal(resolved.interaction?.reply.content, '原话')
    assert.equal(hasUnresolvedAuthoredActions(resolved), false)
    assert.equal(resolved.script, '她发来原话。')
  }
  const ambiguous = resolveAuthoredActions({ script: "<say id='r'>甲</say><say id=\"r\">乙</say>", interaction: { seen: true, reply: { mode: 'immediate', actionId: 'r' } } })
  assert.equal(hasUnresolvedAuthoredActions(ambiguous), true)
  assert.equal(ambiguous.interaction?.reply.content, undefined)
})
...
test('the sole-action rescue stays private-scoped and never grabs a multi-channel action', () => {
  ...
  const withGroup = resolveAuthoredActions({ script: '她回道：<say id="s1">在呢</say>',
    ...
    interaction: { seen: true, reply: { mode: 'immediate', actionId: 'reply' } } })
  assert.equal(hasUnresolvedAuthoredActions(withGroup), true)
  assert.equal(withGroup.interaction?.reply.content, undefined)
  ...
  const withCross = resolveAuthoredActions({ script: '她回道：<say id="s1">在呢</say>',
    ...
    interaction: { seen: true, reply: { mode: 'immediate', actionId: 'reply' } } })
  assert.equal(hasUnresolvedAuthoredActions(withCross), true)
  assert.equal(withCross.interaction?.reply.content, undefined)
})
...
test('new automatic original wins over its unexecuted plan without altering legacy projection', () => {
  ...
  assert.equal(timelineEntryPromptProjection(legacy).content, legacy.content)
})
...
test('compactor exposes provider policy blocks without logging the response body', async () => {
  const error: any = new Error('Bad Request')
  error.response = { status: 400, data: { error: { status: 'PROHIBITED_CONTENT', message: 'content is prohibited under official usage policies' } } }
  const narrator = new OpenAICompatibleNarrator({ http: { post: async () => { throw error } } } as any, {
    providers: [{ label: 'test', enabled: true, endpoint: 'https://example.test/chat', model: 'm', temperature: 0.5, topP: 1, maxTokens: 2048, timeout: 1000, responseFormat: 'json-object', extraHeaders: '', extraBody: '', useForCompaction: true }],
    compaction: { enabled: true, temperature: 0.3, topP: 1, maxTokens: 2048, timeout: 1000, responseFormat: 'json-object' },
    failover: { enabled: false },
  } as any, true)
  await assert.rejects(narrator.compact({ story: request([]).story, from: now, now, entries: [], participants: [], facts: [], scene: null, arc: null } as any), /PROHIBITED_CONTENT/)
})
...
test('delayed background review cannot reopen a completed working detail', async () => {
  ...
  const host: any = {
    dbGetTimeline(...args: any[]) { return this.dbGet(...args) }, memoryConfig: { sceneHookCharacters: 100, sceneSummaryCharacters: 300, arcSummaryCharacters: 300 },
...
test('new private life handoff still feeds Preplan without forwarding private prose or director guesses', async () => {
  ...
  const host = {
    dbGetTimeline(...args: any[]) { return this.dbGet(...args) }, sharedStoryConfig: { shareParticipantDetails: false }, dbGet: async () => [row] }
