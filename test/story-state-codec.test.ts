...
test('director refusal survives restart until its failed window is cleared', () => {
  const state = decodeStoryState({ automation: {
    timelineRetryFrom: '2026-09-15T09:27:28.000Z',
    timelineRetryAt: '2026-09-16T04:00:00.000Z',
    timelineDirectorRefused: true,
    timelineDirectorFailures: 1,
  } })
  assert.equal(state.automation.timelineDirectorRefused, true)
  assert.equal(decodeStoryState(encodeStoryState(state)).automation.timelineDirectorRefused, true)
})
