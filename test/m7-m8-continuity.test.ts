...
test('development projection omits unrelated and other-relationship tendencies', async () => {
  ...
  const service = {
    dbGetTimeline(...args: any[]) { return this.dbGet(...args) }, dbGet: async () => rows }
...
test('failed long-window director degrades to director-less advance instead of freezing', async () => {
  let narrated = false
  const service = { config: { model: { providers: [] } }, schedulePreplanConfig: { enabled: false },
    planAutomaticTimeline: async () => undefined, timelineDirectorFailures: () => 6,
    reportOperation: () => {}, report: (...args: any[]) => { if (args[0] === 'warn') throw args.at(-1) }, mainModelLabel: () => 'mock',
