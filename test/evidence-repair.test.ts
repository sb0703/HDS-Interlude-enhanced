...
test('condition chains retain whole originals, adjacent responses and missing-source markers without leaking branches', async () => {
  ...
  const service: any = {
    dbGetTimeline(...args: any[]) { return this.dbGet(...args) }, dbGet: async (table: string) => table === 'interlude_fact'
...
test('unclassified or imagined completion cannot close a promise, and repeated belief adds no confidence', async () => {
  ...
  let patch: any
  const host: any = {
    dbGetTimeline(...args: any[]) { return this.dbGet(...args) }, memoryConfig: { factContentCharacters: 4000, maxFactsPerStory: 200 },
