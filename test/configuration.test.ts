...
import { Config, inject, RESET_CONFIRMATION_PHRASE, resolveBotScopedTarget, reusable, sharedCommandContext, looksLikeInterludeCommand, version } from '../src/index'
import { normalizeInteraction, hasRequiredNarrativeScript, interludeLoggerName, normalizeGroupVisibleReply, resolveBlindModeConfig, storyBelongsToConfiguredBot, visibleReplyMode } from '../src/service'
...
test('management subcommands bypass the narrative middleware and reach Koishi commands', () => {
  for (const input of ['interlude', 'interlude status', 'interlude.timeline.rebase', '/interlude.timeline.rebase',
    '.interlude.memory.facts', '!INTERLUDE.schedule.refresh']) assert.equal(looksLikeInterludeCommand(input), true, input)
  for (const input of ['interludeX', 'interlude-like', '请执行 interlude.timeline.rebase'])
    assert.equal(looksLikeInterludeCommand(input), false, input)
})
...
test('runtime and plugin exports share one version constant', () => {
  ...
  assert.equal(version, '1.0.1-beta6-rebuild.custom.1')
})
...
test('model Console centralizes connections and task assignment without exposing IDs', () => {
  ...
  assert.deepEqual(Object.keys(model).slice(0, 4), ['vision', 'audio', 'imageGeneration', 'providers'])
