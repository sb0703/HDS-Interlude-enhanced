...
import { NarrativeReview, NarrativeReviewFailure, NarrativeReviewRequest, NarrativeTimeAudit, isFreshNarrativeStart, narrativeReviewInvalidReason, narrativeReviewPrompt, narrativeReviewRepairPrompt, normalizeNarrativeReview, toNarrativeReviewPayload } from './narrative-consistency'
...
import { narrativeDurationAnchors } from './narrative-duration'
...
function narrativeReviewTransportFailure(error: unknown) {
  ...
  if (/^Forbidden$/i.test(String((error as { message?: unknown } | undefined)?.message ?? '').trim())) {
    return { reason: 'http-403', retryable: false }
  }
  if (/^Bad Request$/i.test(String((error as { message?: unknown } | undefined)?.message ?? '').trim())) {
...
/** Inspect only provider error metadata. The response body is never logged: it
 * may contain request excerpts, credentials added by a relay, or private text. */
function providerContentPolicyBlocked(error: unknown) {
  const value = error as {
    message?: unknown
    response?: { data?: unknown }
    cause?: unknown
  } | undefined
  const fragments: string[] = [String(value?.message ?? '')]
  try {
    if (value?.response?.data != null) fragments.push(JSON.stringify(value.response.data))
  } catch {}
  if (value?.cause && value.cause !== error) fragments.push(String((value.cause as any)?.message ?? value.cause))
...
export class OpenAICompatibleNarrator implements NarrativeProvider {
  ...
  async auditTimelinePlan(request: TimelinePlanRequest, plan: TimelinePlan): Promise<'pass' | 'reject' | 'uncertain'> {
    const route = this.routing.timeline.target
    const assigned = this.assignedProviders('compaction')
    const providers = assigned.length ? assigned : this.selectRouteProviders(this.routing.timeline, false)
    const provider = (route.providerId ? providers.find(item => item.id === route.providerId) : undefined) ?? providers[0]
    ...
    const payload = {
      interval: { fromLocal: storyLocalTimeContext(request.from, request.story.setting.timezone).local,
        nowLocal: storyLocalTimeContext(request.now, request.story.setting.timezone).local,
        elapsedMinutes: Math.round(span / 60_000) },
      beats: plan.beats.map(beat => ({ at: beat.at,
        ...
        kind: beat.kind, summary: beat.summary })),
      carry: plan.carry ?? [],
    }
    try {
      return await this.sideTaskJson<'pass' | 'reject' | 'uncertain'>(provider, model, '时间导演校验', this.config.compaction?.timeout || route.timeout || provider.timeout,
        capped => ({
          ...parseObject(provider.extraBody, 'extraBody', this.logger), model, temperature: 0, top_p: 1,
          ...(capped ? { max_tokens: 1200 } : {}), response_format: { type: 'json_object' },
          messages: [
        ...
        text => {
          const value = parseJsonResponse<{ verdict?: unknown }>(text, 'Timeline audit provider')
  ...
  async auditNarrativeTime(request: NarrativeRequest, script: string): Promise<NarrativeTimeAudit> {
    const route = this.routing.compaction.target
    const assigned = this.assignedProviders('compaction')
    const providers = assigned.length ? assigned : this.selectRouteProviders(this.routing.compaction, false)
    const provider = (route.providerId ? providers.find(item => item.id === route.providerId) : undefined) ?? providers[0]
    ...
    const elapsedMinutes = Math.max(0, (request.now.getTime() - request.from.getTime()) / 60_000)
    const durationAnchors = narrativeDurationAnchors(script)
    const prior = request.recentEntries.filter(entry => entry.kind === 'script' && entry.occurredAt <= request.from)
      .sort((a, b) => b.occurredAt.getTime() - a.occurredAt.getTime())[0]
    const previousHandoff = prior ? narrativeEvidence(prior).lifeHandoff : undefined
    const payload = { interval: {
      fromLocal: storyLocalTimeContext(request.from, request.story.setting.timezone).local,
      nowLocal: storyLocalTimeContext(request.now, request.story.setting.timezone).local,
      elapsedMinutes: Math.round(elapsedMinutes * 10) / 10,
    }, freshStart: isFreshNarrativeStart(request), previousHandoff: previousHandoff ? { occurredAtLocal: storyLocalTimeContext(prior.occurredAt, request.story.setting.timezone).local,
      place: previousHandoff.place, activity: previousHandoff.activity } : undefined,
    durationAnchors, script }
    try {
      return await this.sideTaskJson<NarrativeTimeAudit>(provider, model, '主叙事时间校验', this.config.compaction?.timeout || route.timeout || provider.timeout,
        capped => ({
          ...parseObject(provider.extraBody, 'extraBody', this.logger), model, temperature: 0, top_p: 1,
          ...(capped ? { max_tokens: 1600 } : {}), response_format: { type: 'json_object' },
          messages: [
        ...
        text => {
          const value = parseJsonResponse<NarrativeTimeAudit>(text, 'Narrative time audit')
          ...
          if (value.verdict === 'pass' && durationAnchors.length) {
            ...
            if (!Array.isArray(assessments) || assessments.length !== durationAnchors.length
              || new Set(assessments.map(item => item.anchorId)).size !== durationAnchors.length
              || assessments.some(item => !durationAnchors.some(anchor => anchor.id === item.anchorId)
                || !['completed', 'planned', 'remembered', 'ambiguous'].includes(item.relation)))
          ...
          try {
            ...
            const decision = await this.sideTaskJson<AlterAnalysisDecision>(provider, model, 'Alter 分析', alterConfig.timeout ?? route.timeout ?? provider.timeout,
              ...
              text => {
                ...
                const value = parseJsonResponse<AlterAnalysisDecision>(text, 'Alter analysis provider')
            ...
            if (detail.includes('Alter analysis provider returned no description.')
              || /Zhipu request failed \((?:400|401|403)\)/.test(detail)) break
          }
        }
        this.cooldownUntil.set(providerKey(provider), Date.now() + this.config.failover.cooldownMinutes * 60_000)
        if (!this.config.failover.enabled) break
...
export function createCompactor(ctx: Context, config: ModelConfig, silentLogs = false, onUsage?: (record: TokenUsageRecord) => void, routing?: ModelRoutingTable): NarrativeCompactor {
  const resolved = routing ?? resolveModelRouting(config)
  const reviewRoute = config.consistencyReview !== false
    ? resolveModelRouting({ ...config, compaction: { ...config.compaction, enabled: true } } as ModelConfig).compaction
    : resolved.compaction
...
async function requestZhipuStreaming(endpoint: string, body: Record<string, unknown>, headers: Record<string, string>, onText?: (content: string) => Promise<void>, collectUsage?: (usage: unknown) => void) {
  ...
  try {
    ...
    if (!response.ok) {
      // Relay error bodies may echo private request material. Status alone is
      // sufficient for retry decisions and must not leak into runtime logs.
...
function scriptFirstTransportInstruction(phase: NarrativeRequest['phase'], groupTurn: boolean, streaming = false, recovery = false) {
  if (recovery) return [
    ...
    groupTurn ? 'Return groupReply:{"mode":"none"} for silence or groupReply:{"mode":"immediate","content":"exact authored message"} for a completed post.'
      : phase === 'advance' ? 'Keep crossConversationActions participantId and willingness/reason fields. Immediate or delayed actions use their exact content, and delayed actions retain sendAt. Do not invent a current private reply channel.'
      : 'Return interaction:{"seen":true|false,"reply":{"mode":"none"}} for silence, or reply:{"mode":"immediate","content":"exact authored message"}; delayed replies retain content and sendAt. Preserve the intended recipient, complete bubble sequence and configured message separator.',
  ].join('\n')
  const authority = streaming
    ? 'SCRIPT-FIRST TRANSPORT MIRROR: this opt-in streaming path sends the complete interaction.content before script. Preserve those already emitted words in the same causal passage and do not change them afterward.'
    : 'SCRIPT-FIRST TRANSPORT MIRROR: when the script completes visible speech, write its exact words naturally in the living script and copy the same complete words into the matching transport content field. Do not use actionId or <say> markup. A recalled quotation, thought, unsent draft or future possibility stays ordinary prose and does not authorize transport. Multiple bubbles to one recipient form one content string using the configured message separator.'
  if (groupTurn) {
...
export function storyStateForPrompt(state: NarrativeRequest['story']['state']) {
  ...
  const { urge: _urge, historyBackfill: _historyBackfill, timelineBoundary: _legacyBoundary,
    scheduleProfileFingerprint: _legacyFingerprint, ...extensions } = publicState.extensions ?? {}
  const {
  ...
  return { ...publicState, extensions: Object.keys(extensions).length ? extensions : undefined, automation: publicAutomation }
}
  ...
  const payload = {
    ...
    timelinePlan: request.timelinePlan ? {
      beats: request.timelinePlan.beats.map(beat => ({
        at: beat.at,
        hostAtLocal: storyLocalTimeContext(new Date(request.from.getTime()
          + Math.round(Math.max(0, request.now.getTime() - request.from.getTime()) * beat.at)), request.story.setting.timezone).local,
        kind: beat.kind,
...
export function toTimelinePlanPayload(request: TimelinePlanRequest) {
  ...
  return {
    ...
    recentScriptContinuation: request.recentScriptContinuation ? { ...request.recentScriptContinuation, content: request.recentScriptContinuation.content.slice(-600) } : undefined,
...
export function toCompactionPayload(request: CompactionRequest) {
  ...
  // Full author canon is already used by live narration and the independent
  // schedule reviewer. Re-sending it on every background compaction both
  // duplicates large private data and can cause an unrelated historical batch
  // to be rejected by a provider policy filter. Keep only structural identity
  // and evidence-backed development candidates here.
  ...
  return {
    ...
    setting: {
      ...
      relationship: '',
      world: '', perspective: '', supportingCast: '', location: '', style: '',
