...
import { createCompactor, createEmbedder, createImageGenerator, createNarrator, createStickerDescriber, createVisionDescriber, formatTokenUsageLine, ImageGenerator, ModelConfig, promptVisibleMessageContent, recentScriptOwnership, StickerDescriber, TokenUsageRecord, usesRemoteProviders, VisionDescriber, AudioConfig, compactPromptEntries, StickerDescription } from './narrator'
...
import { authoredActionRecoveryDraft, hasUnresolvedAuthoredActions, recoverExplicitAuthoredMirror, resolveAuthoredActions } from './script/authored-actions'
...
import {
  CompactionDecision, CompactionRequest, emptyStorySetting, emptyStoryState, ImageSubject, IntentDraft, InterludeArc, InterludeScene,
  ...
  PreviousSceneSummary, RecalledMoment, WorkingDetail, TimelinePlan, TimelinePlanRequest,
...
/** A policy rejection is tied to the story's current sensitive evidence, not
 * only to one exact entry fingerprint. New chat rows must not bypass cooldown
 * and create a request storm against the same blocked material. */
const COMPACTION_POLICY_RETRY_BACKOFF = 6 * Time.hour
...
/** A definite upstream denial is not a transient failed writing turn. */
export function isNonRetryableNarrativeRefusal(error: unknown) {
...
export function interludeLoggerName(config: Pick<Config, 'onebot' | 'storyDefaults'>) {
  const accounts = (config.onebot?.botAccounts ?? []).filter(account => account.enabled !== false)
    .map(account => normalizeAccountId(account.qq)).filter(Boolean)
...
function recordInterludeRuntimeLog(config: Pick<Config, 'onebot' | 'storyDefaults'>, level: InterludeRuntimeLogEntry['level'], message: string) {
  const botId = (config.onebot?.botAccounts ?? []).filter(account => account.enabled !== false)
    .map(account => normalizeAccountId(account.qq)).find(Boolean) || 'unconfigured'
...
export function storyBelongsToConfiguredBot(story: Pick<InterludeStory, 'platform' | 'selfId'>, onebot?: OneBotNapCatConfig): boolean {
  // Explicit bot ownership must survive a disabled user allowlist gate.
  // Otherwise enabling a second instance can archive the first bot's story.
  if (!onebot?.enabled && !onebot?.botAccounts?.length) return true
  const enabledBots = (onebot?.botAccounts ?? []).filter(account => account.enabled !== false)
    .map(account => normalizeAccountId(account.qq)).filter(Boolean)
...
export class InterludeService extends Service {
  ...
  async resetAllRuntimeData(): Promise<FullResetResult> {
    ...
    try {
      const tables = ['interlude_script_entry', 'interlude_memory', 'interlude_intent', 'interlude_scene', 'interlude_arc', 'interlude_fact', 'interlude_state_patch', 'interlude_overlay_snapshot', 'interlude_web_observation', 'interlude_schedule_preplan'] as const
    ...
    try {
      ...
      const fill = async (row: ScriptEntry) => {
        ...
        const applied = await this.serial(storyId, async () => {
          ...
          const [fresh] = await this.dbGetTimeline('interlude_script_entry', { storyId, id: row.id }) as ScriptEntry[]
  ...
  private async advanceUnlocked(story: InterludeStory, now: Date, force: boolean) {
    ...
    if (!force && story.state.automation?.timelineDirectorRefused
      && timelineRetryFrom === from.toISOString() && timelineRetryAt && timelineRetryAt > now) {
  ...
  private async planAutomaticTimeline(story: InterludeStory, participant: InterludeParticipant | null, phase: Extract<NarrativeRequest['phase'], 'advance' | 'conversation-follow-up' | 'intent-due'>, from: Date, now: Date, dueIntents: NarrativeIntent[], recovery?: string, bypassRetryGate = false) {
    ...
    try {
      ...
      const validatePlan = async (candidate: TimelinePlan | undefined) => {
      ...
      if (windowIssues.length) plan = undefined
      if (plan) {
        await this.clearTimelineDirectorRetry(story, now)
      ...
      await this.persistTimelineDirectorRetry(story, from, now, failures, refusal ? badRequest ? 40 * Time.minute : TIMELINE_DIRECTOR_FUSE_COOLDOWN : undefined, refusal)
      return undefined
    }
  }
  ...
  private async persistTimelineDirectorRetry(story: InterludeStory, from: Date, now: Date, failures: number, delayOverride?: number, refused = false) {
    ...
    const automation = {
      ...
      timelineDirectorRefused: refused || undefined,
  ...
  private async reviewCandidate(context: NarrativeRequest, raw: NarrativeDecision, retrievalHints: Array<{ previousId: number; similarity: number }>) {
    ...
    const reviewDelivery = (target: string, content: string): ReviewDelivery => {
      const separator = this.config.runtime.messageSeparator?.trim() || '<sep/>'
      const bubbles = this.config.runtime.splitReplyMessages === false
        ? [content]
        : content.split(separator).map(part => part.trim()).filter(Boolean)
    ...
    if (phase === 'advance' || phase === 'user-message') {
      allowedDeliveries.push(...decision.crossConversationActions.filter(action => action.mode === 'immediate')
        .map(action => reviewDelivery(action.participantId, action.content)))
    }
    ...
    const request: NarrativeReviewRequest = { context, candidate: raw, allowedDeliveries, alreadyDelivered, retrievalHints, requireSemanticChecks: true, requireDeliveryCheck: true, evidenceCharacterBudget: this.config.model.consistencyReviewHistoryCharacters,
  ...
  private async tryDecide(story: InterludeStory, participant: InterludeParticipant | null, phase: NarrativeRequest['phase'], from: Date, now: Date, userMessage: string | undefined, dueIntents: NarrativeIntent[], supersededIntents: NarrativeIntent[] = [], groupContext?: GroupContext, images: NarrativeImage[] = [], audio: NarrativeAudio[] = [], chatCapabilities?: ChatActionCapabilities, quotedMessages: IndexedQuotedMessageContext[] = [], stickerCatalog: StickerCatalogEntry[] = [], turnQueryEmbedding?: number[], visualObservations?: string[], onEarlyReply?: (reply: EarlyNarrativeReply) => Promise<boolean>, options: { bypassTimelineBackoff?: boolean } = {}): Promise<{ decision: NarrativeDecision; succeeded: boolean; effectiveNow: Date; immediateObservations: WebObservation[]; timelinePlan?: TimelinePlan; failureReason?: NarrativeFailureReason }> {
    ...
    let timelinePlan = automaticPhase
      ...
      : undefined
    if (automaticPhase && !timelinePlan && story.state?.automation?.timelineDirectorRefused
      && story.state.automation.timelineRetryFrom === from.toISOString()) {
    ...
    try {
      ...
      const generateRaw = (outputRecovery = false, recovery = '', outputRecoveryDraft?: NarrativeDecision) => outputRecoveryDraft
        ...
        : this.decide(story, participant, phase, from, effectiveNow, userMessage, dueIntents, supersededIntents, groupContext, images, audio, immediateObservations, outputRecovery, chatCapabilities, quotedMessages, stickerCatalog, turnQueryEmbedding, visualObservations, timelinePlan, timelineFallback, earlyReply, recovery, captureContext)
      // A delayed-reply intent already contains an exact, user-authorized
      // transport payload. Its due turn may narrate the surrounding life, but
      // a weak model must not be able to erase or replace that queued message.
      const generate = async (outputRecovery = false, recovery = '', outputRecoveryDraft?: NarrativeDecision) =>
      ...
      if (initialVisibleRecovery) {
        ...
        this.reportOperation('standard', 'warn', story, phase,
          hasUnresolvedAuthoredActions(decision)
            ? '动作引用失配，已保留未落库草稿并定向修复传输结构'
        ...
        if (requiresVisibleReplyRecovery(phase, groupContext, decision)) {
          this.reportOperation('standard', 'warn', story, phase, '传输结构修复失败 类型=%s 回复模式=%s',
            hasUnresolvedAuthoredActions(decision) ? 'unresolved-action-reference' : 'missing-or-invalid-transport', visibleReplyMode(decision, phase, groupContext))
      ...
      for (let attempt = 0; ; attempt++) {
        ...
        // A live user turn can spend seconds generating and repairing the
        // unpublished draft. The host owns those elapsed seconds too: audit
        // against the actual review time, while history rows keep their exact
        // arrival timestamps. Automatic turns retain their planned endpoint.
        ...
        // The broad reviewer has accepted out-of-window prose in production.
        // A separate, narrow audit must agree before the script is committed.
        const timeAudit = usesRemoteProviders(this.config.model) && this.compactor.auditNarrativeTime && reviewContext
          ? await this.compactor.auditNarrativeTime(reviewContext, decision.script ?? '') : undefined
  ...
  private async persistDecision(
    ...
    const executingDelayedIntent = phase === 'intent-due' && participant && decision.interaction?.reply.mode === 'immediate'
      ? contextIntents.find(intent => intent.type === 'delayed-reply'
        && intent.participantId === participant.id
        && normalizeMessageSeparators(intent.payload?.content, this.config.runtime.messageSeparator).trim() === decision.interaction!.reply.content)
      : undefined
    // The original delayed event is the authoritative transport event. The
    // due passage may describe its execution, but must not create a second
    // outgoing ledger action for the same visible message.
    const commitDecision = executingDelayedIntent
      ? { ...decision, interaction: { seen: false, reply: { mode: 'none' as const } } }
      : decision
    ...
    if (participant && permitMessages && !immediateReplyAlreadyDelivered && interaction?.reply.mode === 'immediate' && interaction.reply.content) {
      ...
      messages.push(executingDelayedIntent
        ? { ...message, scriptEvent: restoreMessageEvent(executingDelayedIntent.payload, interaction.reply.content) }
        : message)
    }
  ...
  private async contactThreads(storyId: string, selected: NarrativeFact[], participantId?: string) {
    ...
    if (linkedIds.length) {
      const archived = await this.dbGetTimeline('interlude_fact', { storyId, id: { $in: linkedIds } }) as NarrativeFact[]
    ...
    const entries = await this.dbGetTimeline('interlude_script_entry', { storyId, id: { $in: ids } }) as ScriptEntry[]
  ...
  private async updateScriptDeliveryOutcome(
    ...
    try {
      const entry = (await this.dbGetTimeline('interlude_script_entry', { storyId, id: reference.scriptEntryId }))[0] as ScriptEntry | undefined
  ...
  private async recordAutomaticDelivery(
    ...
    try {
      ...
      if (delivery.sourceEntryId) {
        const entry = (await this.dbGetTimeline('interlude_script_entry', { storyId, id: delivery.sourceEntryId }))[0] as ScriptEntry | undefined
        const actions = entry?.metadata?.deliveryActions
  ...
  private async deferFailedAutomaticAdvance(storyId: string, now: Date, failedFollowUp: boolean) {
    const config = this.autoAdvanceConfig
    if (!config.enabled) return
    const story = await this.getStory(storyId)
    const intervalMinutes = automaticIntervalMinutes(story, now, config)
    const ordinaryRetry = new Date(now.getTime() + intervalMinutes * Time.minute)
    const retryAt = await this.schedulePreplanAnchoredTime(story, now, ordinaryRetry)
    const futureFollowUps = (story.state.automation?.conversationFollowUpAt ?? [])
      .map(toDate)
      .filter((value): value is Date => !!value && value > now)
      .sort((left, right) => left.getTime() - right.getTime())
    const conversationFollowUpAt = failedFollowUp
      ? [retryAt, ...futureFollowUps.filter(value => value.getTime() !== retryAt.getTime())]
      : futureFollowUps
    const automation = {
      ...(story.state.automation ?? {}),
    ...
    this.reportOperation('standard', 'warn', story, 'advance',
      '自动写作失败已退避，下次重试=%s 间隔=%d分钟',
      formatLogTime(retryAt, story.setting.timezone), Math.max(1, Math.round((retryAt.getTime() - now.getTime()) / Time.minute)))
  }
  ...
  private async prepareCompaction(story: InterludeStory, now: Date, force: boolean): Promise<PreparedCompaction | undefined> {
    ...
    const rawDevelopmentCandidates = (await this.dbGetTimeline('interlude_state_patch', { storyId: story.id, status: { $in: ['proposed', 'applied'] } },
    ...
    const candidateSources = candidateSourceIds.length
      ? await this.dbGetTimeline('interlude_script_entry', { storyId: story.id, id: { $in: candidateSourceIds } }) as ScriptEntry[]
      : []
  ...
  private async persistCompaction(story: InterludeStory, scene: InterludeScene, decision: CompactionDecision, entries: ScriptEntry[], now: Date, visibleFactIds = new Set<number>(), reviewedPresence: ScenePresenceState[] = []) {
    ...
    for (const draft of decision.episodeTags ?? []) {
      ...
      const current = (await this.dbGetTimeline('interlude_script_entry', { storyId: story.id, id: source.id }))[0] as ScriptEntry | undefined
      if (!current) continue
    ...
    if (explicitBoundary) {
      ...
      if (boundaryEntry) {
        const source = (await this.dbGetTimeline('interlude_script_entry', { storyId: story.id, id: boundaryEntry.id }))[0] as ScriptEntry | undefined
    ...
    if (decision.workingDetails?.length) {
      ...
      for (const draft of decision.workingDetails) {
        ...
        if (label && (draft.resolved === true || draft.status === 'resolved')) { merged.delete(label); resolutions[label] = revision; continue }
  ...
  private async developmentForPrompt(storyId: string, participantId: string | undefined, query: string) {
    ...
    const rows = await this.dbGetTimeline('interlude_state_patch', { storyId, status: 'applied' }, { limit: 100, sort: { appliedAt: 'desc' } }) as StatePatchProposal[]
...
function normalizeVisibleMessageContent(value: unknown, maxCharacters: number, separator = '<sep/>') {
  return normalizeMessageSeparators(value, separator)
    ...
    .replace(/\[CQ:(?:file|image|record|audio|video|flash|mface)(?:,[^\]]*)?\]/gi, '')
...
/** Structural source grounding; semantic status requires independent review. */
...
export function normalizeNarrativeImageGeneration(value: unknown): NarrativeDecision['imageGeneration'] {
  if (!isRecord(value) || typeof value.prompt !== 'string') return undefined
  const prompt = value.prompt.trim().slice(0, 2_000)
  const subject = value.subject
  if (!prompt || subject !== 'protagonist' && subject !== 'other-person' && subject !== 'non-person') return undefined
...
const normalizeStoryState = decodeStoryState
...
export function groupDueIntents(intents: NarrativeIntent[]) {
  ...
  for (const intent of [...intents].sort((left, right) => left.notBefore.getTime() - right.notBefore.getTime() || left.id - right.id)) {
    // Every delayed reply owns one immutable payload and one delivery-ledger
    // reference. Execute it in its own batch so several overdue drafts cannot
    // be merged into one message or marked complete together.
    const family = intent.type === 'proactive-check' ? 'agency'
      : intent.type === 'delayed-reply' ? `delayed:${intent.id}` : 'normal'
...
/** The host computes each proposed beat's actual local clock. Model text can
 * describe an event, but it cannot move that event to a later daypart or clock.
 * These are general clock expressions, independent of any character routine. */
export function timelinePlanWindowIssues(plan: TimelinePlan, from: Date, now: Date, timezone: string) {
  const span = Math.max(0, now.getTime() - from.getTime())
  const dayparts: Array<{ label: string; pattern: RegExp; contains: (hour: number) => boolean }> = [
  ...
  return plan.beats.flatMap((beat, index) => {
    ...
    const clock = storyLocalTimeContext(at, timezone)
    const issues: string[] = []
    for (const part of dayparts) {
      if (part.pattern.test(beat.summary) && !part.contains(clock.hour)) {
        issues.push(`beat ${index + 1} ${part.label} != ${clock.local}`)
      }
    }
    for (const match of beat.summary.matchAll(/(?<!\d)([01]?\d|2[0-3])[:：]([0-5]\d)(?!\d)/gu)) {
      const claimed = Number(match[1]) * 60 + Number(match[2])
      const actual = Number(clock.time.slice(0, 2)) * 60 + Number(clock.time.slice(3, 5))
      // A beat may span a few minutes of rounding or a local clock quote;
      // beyond that, the stated clock belongs to a different window.
      if (Math.abs(claimed - actual) > Math.max(2, Math.ceil(span / 60_000))) {
        issues.push(`beat ${index + 1} clock ${match[0]} != ${clock.local}`)
      }
    }
    return issues
  })
...
export function bindDueDelayedReplyExecution(
  decision: NarrativeDecision,
  phase: NarrativeRequest['phase'],
  participantId: string,
  ...
  const intent = dueIntents.find(item => item.type === 'delayed-reply'
    && item.participantId === participantId
    && item.payload?.interaction === true)
  const content = normalizeMessageSeparators(intent?.payload?.content, separator).trim()
  if (!content) return decision
  return {
    ...decision,
    interaction: { seen: false, reply: { mode: 'immediate' as const, content } },
  }
}
