...
/** What the generated image visibly depicts. Only protagonist images may use
 * the canonical character reference; the other values deliberately stay on
 * the ordinary text-to-image route. */
export type ImageSubject = 'protagonist' | 'other-person' | 'non-person'

export interface OutgoingMessageDraft {
  participantId: string
  content: string
  /** A bounded, already-authorized visual prompt to generate and attach to this immediate delivery. */
  imagePrompt?: string
  imageSubject?: ImageSubject
  /** Extracted from the canonical character profile, never generated by the narrator. */
...
export interface QuotedMessageContext {
  ...
  speaker: string
  content: string
  /** Transport-authenticated metadata used only for durable quote evidence. */
  messageId?: string
  occurredAt?: string
}
...
export interface NarrativeDecision {
  ...
  /** One paid image attached to the turn's delivered immediate message: a user-requested reply, an agency-approved proactive contact, or an advance cross-conversation photo send. */
  imageGeneration?: { prompt: string; subject: ImageSubject }
...
export interface NarrativeRequest {
  ...
  now: Date
  userMessage?: string
  /** Whether this request may ask the host to create one image. */
  imageGenerationEnabled?: boolean
  /** Whether an eligible portrait-reference image has been configured. */
