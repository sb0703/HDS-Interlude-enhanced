...
export function applySchedulePreplanProposal(
  ...
  // Failure must not stamp today as successfully reviewed. The caller retains
  // the existing row and applies its bounded retry backoff.
...
export function schedulePreplanWindow(record: SchedulePreplanRecord | undefined, now: Date, timezone: string, hours = 12, config: Pick<SchedulePreplanConfig, 'candidateActivationProbability' | 'candidateRevealMinutes'> = DEFAULT_SCHEDULE_PREPLAN_CONFIG, from = now): SchedulePreplanWindow | null {
  ...
  const recentBlocks: SchedulePreplanWindow['blocks'] = []
  ...
  const days = new Map(materializeSchedulePreplan(record.regimes, record.exceptions, addDate(today, firstOffset), Math.ceil(endMinute / 1440) - firstOffset).map(day => [day.date, day]))
  for (const day of record.materializedDays) days.set(day.date, day)
    ...
    for (const block of day.blocks) {
      ...
      if (block.tentative) {
        ...
        if (minutesUntil > config.candidateRevealMinutes) {
          destination.push({ ...block, label: '可能的个人安排', location: undefined, date: day.date, tentative: true })
          continue
        }
      }
...
function normalizeProposal(value: unknown, validEvidenceIds: ReadonlySet<number>, variationLevel: SchedulePreplanConfig['variationLevel'], authorProfileEvidence = false): SchedulePreplanProposal | undefined {
  ...
  if ((outcome === 'replace' || outcome === 'patch') && !Array.isArray(value.regimes)) return undefined
  if (Array.isArray(value.regimes) && value.regimes.length && !regimes.length) return undefined
