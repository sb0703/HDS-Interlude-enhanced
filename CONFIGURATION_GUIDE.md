...
安装并启用 Koishi Console 后，侧边栏会出现 **HDSI 重置** 页面。点击“一键重置全部故事”并确认后，HDSI 会从当前 `storyDefaults` 和账号资料重新建立人物 Canon、Perspective、关系和世界，同时清理时间线、场景、记忆、事实、意图、Overlay、Agency 与 Alter 状态。

模型连接、API Key、OneBot / NapCat、图片生成、人物参考图和 Console 中的 `storyDefaults` 不会被修改。由于旧聊天和记忆不会保留，请确认当前 `storyDefaults` 已填写正确后再执行。

### 多账号、多角色

可在 Koishi 的同一分组下新增多个 HDSI 实例，每个实例只启用一个不同的 OneBot 机器人 QQ，并分别填写自己的 `storyDefaults`、模型与人物参考图。运行时会按机器人 QQ 隔离 Canon、剧情、记忆、自动推进和压缩；任何一个实例都不会归档、推进或重置另一个 QQ 的剧本。

不要让两个 HDSI 实例同时启用同一个机器人 QQ；那会造成同一条消息被两个角色同时接收。Console 的 **HDSI 重置** 页面会先让你选择目标角色，重置只影响该账号对应的剧本。
...
`canonGuard.enabled` 开启后，每份主叙事草稿都会在发送和写入数据库前接受一次独立 Canon 一致性检查。检查范围包括角色资料中的明确身份事实、性取向、职业、地点、关系边界、禁止事项和具体工作日日程。违规草稿不会发送，也不会进入剧本；插件按 `maxRewriteAttempts` 要求模型重写，仍不合规则让本轮失败并走现有安全重试。该功能会增加模型调用次数，严格角色场景建议开启。

`canonGuard` 不会把“通常、偶尔、很少、可能”等软习惯错误地当成绝对禁令。明确日程只有在当前上下文已经存在请假、出差、紧急事项、外出预约或计划变更等具体原因时才允许例外；模型不能在违规草稿里临时编造理由为自己开脱。
