...
`agency.capacityPolicy` 默认 `contextual`。模型可为本次联系提供 `proactiveContact.capacityReason`，解释忙碌间隙、设备如何可用、当前消息如何保障隐私；统一一致性审核会独立检查，理由本身不是放行证明。`conservative` 保留原有固定条件。关闭统一审核时自动使用保守条件。

两种策略均不绕过设备完全不可用、目标权限、意愿门槛或联系间隔。普通联系间隔由配置决定，不由角色描述自行取消。
