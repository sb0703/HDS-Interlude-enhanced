...
## 1.0.1-beta6-rebuild.custom.1

- 时间导演不再自行计算相对时间比例，只返回有序事件；宿主按真实叙事窗口动态分配节点位置，并把每个节点的精确 `hostAtLocal` 传给主叙事，避免导演与主叙事重复换算本地时钟。
- Alter 分析只接受非空 `description`；缺失字段时进行一次无输出上限恢复，智谱 400/401/403 不再重复消耗请求，错误日志不再包含服务商响应正文。
- 时间导演在候选节点的时段或时钟越过宿主窗口时拒绝落库并定向重写；重写遇到 HTTP 400/403 或内容审核拒绝时按上游拒绝处理，保留故事游标并进入冷却，避免错误计划写入或高频重试。
- 延迟回复到期时使用数据库中已授权的原始 payload 强制绑定即时投递，模型只能补写周围剧情，不能清空、替换或再次延迟消息；实际回执继续更新最初的 pending 剧本事件。多个到期延迟回复分别执行，避免合并内容或批量误结算。延迟计划创建时的正文必须停在发送动作之前。
- 正常主叙事输出改为直接在 `content` 中镜像正文消息，不再要求模型生成跨字段 `actionId`/`<say>` 引用；旧格式仅保留兼容解析和受控恢复。统一审核的示例补齐必需的 delivery 检查，模型返回格式不再因示例与文字要求冲突而高频失败；无效的可选 `reportedTimes` 提取会被丢弃，不再阻断已有完整证据的审核。
- 结构恢复保留解析前的原始模型草稿；恢复提示统一使用正文镜像字段并省略 actionId，修复草稿位置引用错误，兼容 say 标签的单引号及空白变体。原草稿含多段明确 say 行动且内容镜像逐字一致时，本地只修正引用而不再次创作；其他情况仍须受控恢复并通过投递一致性审核。
- 动作引用失配保留为待修复状态，不再静默降级为不回复；修复一次后仍失配则拒绝落库。正常叙事审核增加必须引用传输证据的 delivery 结论，正文声称向真实参与者发消息但无对应投递时必须拒绝并重写。记忆压缩审核保持原有契约。
- 适配上游 V2 `f9eda0c` 的提交/投递事件、场景交接、原文记忆导航、版本化状态和原生音频输入。
- 保留双 bot 故事归属、独立图片与参考图、账号日志和带原文依据的动态审核；未采用上游固定词表的时间越界检测。
- 保留旧时间线边界、日程指纹与导演失败计数；防止重建前的向量回填写回旧进度，防止旧缓存加载失败影响新缓存，并从提示词排除主机恢复控制字段。

## 0.1.5-beta8-m6.custom.4

- 统一逻辑审核增加安全的失败阶段与错误码，区分模型请求、审核结构修复和时间提取复核失败；诊断不包含接口密钥或响应正文。单次审核遇到临时网络故障、HTTP 408/429/5xx 最多重试一次，401/403 不重试。独立时间提取格式无效时最多修复一次，仍无有效依据则继续阻止写入。故障恢复可能增加模型调用次数。
- 统一逻辑审核的 `interval` 证据新增 `storyTimezone`、`fromLocal`、`nowLocal`、`fromLocalContext` 与 `nowLocalContext`，日期、星期和早晚判断统一使用故事本地时间。
- 审核提示明确禁止把 ISO 时间尾部的 `Z` 当作故事所在地时钟；当前事件、群消息和历史条目同步附带本地时间。

## 0.1.5-beta8-m6.custom.3

- 可见回复结构恢复不再独立重写整个回合：恢复请求携带上一份未落库草稿，使用零温度定向补齐 `interaction` 或 `groupReply`，并保留原剧本与其他有效字段。
- 恢复后仍不满足显式传输合约时继续拒绝落库，不会伪造或默认发送消息。

## 0.1.5-beta8-m6.custom.2

- 逻辑审核返回格式不符合严格落库契约时，增加一次受限的结构修复请求。
- 修复请求仍要求候选原文逐字引用与现有证据编号，不把无依据审核视为通过。
- 审核格式失败日志现在输出不含剧情正文的原因码，便于区分 JSON、原句、证据和字段问题。

## 0.1.5-beta8-m6.custom.1

- 时间导演新增故事时区、本地日期时间和完整本地时间上下文；UTC 与故事本地时间同时保留，但日期、星期和早晚判断统一以本地字段为准。
- 时间计划解析兼容数字、字符串、百分比及中英文事件类型别名，未知类型继续拒绝；失败按 10、20、40、80、120、120 分钟退避并持久化，连续六次失败后仅允许通过统一审核的守恒小步推进，手动推进可主动探测并解除熔断。
- 场景压缩增加条目范围指纹、两小时同指纹冷却和持久化检查点复核；新条目与手动整理可立即重试，Schedule Preplan 审核结果不再被场景压缩失败连带丢弃。
- 模型可见消息在裁剪与拆分前统一规范 `<sep/>` 的大小写、空格、全角和自闭合变体；不修改用户原文与旁白正文。
- 新增默认启用的动态 ChatRhythm：只记录成功投递回复的气泡数量、长度档位、收尾类型和总字数，按当前内容提示自然改变表达节奏；不写死性别、地点、关系或固定消息模板，也不增加模型调用。

## 0.1.4

- 移除主流程的固定工作周日程解析；首次使用或人物设定/时区变化后，使用现有日程模型重新解释当前设定，不要求固定标题或格式。保留已有日期例外；同步失败不向本轮提供旧日程，且使用退避重试。后台旧版本结果不能覆盖新日程。
- 时间提取支持相对日期和明确日期，模糊十二小时读数保留歧义，不再把价格当时间或自动猜上午/下午；正文时钟按整个叙事区间解释。统一审核开启时，时钟关键词只提供线索，不单独否决审核。日照不再由钟点断言。
- 在场记忆更新要求原文引用并复用统一审核检查人物与动作对应关系，替代全段关键词判定；审核不可用时保留原状态。仅后台提出有依据的在场更新时增加此审核，不额外阻断其他记忆整理。
- 新增 `agency.capacityPolicy`，默认按情境判断；具体联系可提供 `capacityReason` 供独立审核，不再把忙碌或共享环境一概当作不能联系。关闭统一审核时回到保守策略；设备不可用、参与者权限和联系间隔仍保留。新增审核历史字符预算配置，优先引用事件并保留长记录结尾。
- 新增默认开启的 `model.consistencyReview`：复用压缩模型，使用本轮角色设定、已过滤历史、事件计划和允许的投递动作统一审核，不再由场景关键词决定事件是否合理。正常每轮增加一次审核；发现问题最多重写一次，计划本身错误时先重新规划。审核不可用、结果无依据或重写仍矛盾时不落库、不投递、不推进游标；不会从正文提取消息补发。
- 统一审核启用时先审后发，暂停实验性流式首条投递，并替代单独 Canon 守卫，避免重复审核。旧的场景回环、活动生命周期和日程关键词函数保留兼容，但不再进入主生成拦截链；文本相似度仅作为审核线索。审核仍可能误判，不能保证所有自然语言矛盾都被发现。
