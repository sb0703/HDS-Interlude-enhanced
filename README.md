...
- **侧端失败保护**：Alter 只接受非空 `description`，结构缺失时进行一次无输出上限恢复，并对鉴权或请求拒绝及时停止重复调用；时间导演会在候选计划越过宿主时间窗口时拒绝落库，上游返回 400/403 或内容审核拒绝时保留游标并进入冷却。
...
本轮验证包含 443 项本地测试、TypeScript 类型检查，以及两条配置路线各 9 项纯虚构模型案例。模型服务不可用或审核结构仍无效时，保留未提交状态等待重试；验证结果不代表所有自然语言场景都能正确判断。技术说明见[审核与恢复边界](docs/development/2026-09-08-review-boundary-cleanup.md)。
...
HDSI 支持在同一个 Koishi 分组中启用多个插件实例。推荐每个实例绑定一个独立的 OneBot / QQ 机器人账号，并分别配置自己的 `storyDefaults`、模型、白名单和人物参考图。

运行时会按机器人 QQ 隔离 Canon、剧情、参与者、记忆、自动推进、压缩任务和日志。一个实例只处理自己绑定的机器人账号，不会推进、归档、重置或读取另一个 QQ 的故事；多个实例可以使用不同人物和不同剧本同时运行。

不要让两个 HDSI 实例绑定同一个机器人 QQ，否则同一条消息可能被两个角色同时处理。两个 NapCat / OneBot 账号也必须分别登录，并使用各自对应的连接配置和 Token。
...
如果需要同时运行两个角色，请在 Koishi 中新增两个 HDSI 实例：

1. 实例 A 的 `onebot.botAccounts` 只填写 QQ A，并配置角色 A 的 `storyDefaults`；
2. 实例 B 的 `onebot.botAccounts` 只填写 QQ B，并配置角色 B 的 `storyDefaults`；
3. 两个实例分别填写对应的 `userAccounts`、模型和人物参考图；
4. 确认 QQ A、QQ B 都已在各自的 NapCat 中登录，且 OneBot 连接状态均为绿色。

配置完成后，可分别在两个 QQ 私聊中执行 `interlude.doctor` 检查当前机器人账号、角色档案、模型和运行中故事。若某个账号未登录，该账号对应实例不会收到消息，不能据此判断多账号隔离失效。
...
人物参考图只用于画面中明确出现主角本人时的剧情生图，包括主角与其他人的合照。风景、食物、宠物、物品、其他人物，以及主角没有入镜的第一视角照片，不会套用主角参考图，以免把主角形象错误带入无关画面。

该规则同时适用于用户索要图片、主角主动分享图片和后台剧情投递。参考图只约束主角，不会改变合照中的其他人物。人物参考图配置与聊天模型、压缩模型、Embedding 和视觉识别模型相互独立。
...
Koishi Console 还提供 **HDSI 重置** 页面。选择目标角色并确认“重置全部故事”后，只会清理该机器人账号对应的 Canon、Perspective、关系、世界、时间线、场景、记忆、意图、Overlay、Agency 和 Alter 状态，然后按当前 `storyDefaults` 重新建立故事。模型连接、API Key、OneBot / NapCat、图片生成和人物参考图配置不会被修改。
