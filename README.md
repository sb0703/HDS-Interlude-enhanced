...
本轮验证包含 446 项本地测试、TypeScript 类型检查，以及两条配置路线各 9 项纯虚构模型案例。模型服务不可用或审核结构仍无效时，保留未提交状态等待重试；验证结果不代表所有自然语言场景都能正确判断。技术说明见[审核与恢复边界](docs/development/2026-09-08-review-boundary-cleanup.md)。
